...
- 網断で half-open になった master（`-O check` は成功するが実通信が返らない）
  も 60 秒ごとの死活プローブで検知し、連続 2 サイクル不調なら自動で畳んで
  再確立 → forward を復旧（v0.13）。すべての ssh 子プロセスにタイムアウトが
  付いたため、修復処理自体が固まることもない
